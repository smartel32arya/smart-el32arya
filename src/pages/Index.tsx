...
const Index = () => {
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      {/* Why Us Section */}
      <section className="py-16 md:py-28 bg-gradient-to-br from-secondary via-background to-secondary relative overflow-hidden">
        ...
        <div className="container relative z-10">
          ...
          {/* Stats */}
          <div className="grid grid-cols-3 gap-4 md:gap-8 max-w-4xl mx-auto mb-10 md:mb-16">
            {[
              ...
              <div
                key={stat.label}
                className="bg-white rounded-2xl p-4 sm:p-6 md:p-8 border border-border shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 text-center group"
              >
                <div className="text-2xl sm:text-3xl md:text-4xl mb-2 sm:mb-3 group-hover:scale-110 transition-transform">{stat.icon}</div>
                <div className="text-gold font-black text-xl sm:text-3xl md:text-4xl mb-1 sm:mb-2">{stat.num}</div>
                <div className="text-muted-foreground font-semibold text-xs sm:text-sm md:text-base">{stat.label}</div>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 max-w-6xl mx-auto">
            {[
              ...
              <div
                key={feature.title}
                className="bg-white rounded-xl p-5 md:p-6 border border-border hover:border-gold/40 transition-all duration-300 group hover:shadow-lg"
              >
                <div className="text-2xl md:text-3xl mb-2 md:mb-3 group-hover:scale-110 transition-transform">{feature.icon}</div>
                <h3 className="font-bold text-base md:text-lg text-foreground mb-1 md:mb-2">{feature.title}</h3>
